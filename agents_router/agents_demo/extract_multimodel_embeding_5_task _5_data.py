...
# -- 每个任务最多处理的 episode 数（按 episode*.mp4 排序取前 N） --
VIDEO_PER_TASK     = 5

# -- 数据源：任务 -> (指令, 某个该任务视频的路径【用于定位目录】) --
VIDEOS = {
    "adjust_bottle": (
...
import os, json, math, subprocess, glob
...
def _list_task_videos(seed_path: str, limit: int):
    """
    给定某个任务的任意一个 episode 路径（如 episode0.mp4），
    在同目录下匹配 episode*.mp4，按文件名排序取前 limit 个。
    若目录里没有匹配，则若 seed_path 是文件且存在，则只返回 seed_path。
    """
...
def main():
    ...
    for task_key, (instruction, seed_vpath) in VIDEOS.items():
        # 列出该任务的前 N 个 episode
        ...
        for vpath in video_list:
            ...
            print(f"[Info] 提取首帧: {task_key}/{episode_stem} -> {frame_path}")
            try:
    ...
    print(f"[OK] 共生成 {total} 条记录（目标上限：{VIDEO_PER_TASK} × {len(VIDEOS)}）")
